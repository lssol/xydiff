...
void NodesManager::PrintAll(void)
{
	...
	{
		if (!v0Assigned(i))
			vddprintf(("%4d not matched(delete)\n", (int)i));
		// else printf("%4d[%s]->%4d\n", (int)i, v0node[i].tag.c_str(), (int)v0node[i].myMatchID);
	...
	{
		if (!v1Assigned(i))
			vddprintf(("%4d not matched(insert)", (int)i));
		else
...
{
	if ((id < 1) || ((unsigned int)id > sourceNumberOfNodes))
		throw OutOfBound();
	return (v0node[id].myMatchID != NULL_ID);
}
bool NodesManager::v1Assigned(int id)
{
	if ((id < 1) || ((unsigned int)id > resultNumberOfNodes))
		throw OutOfBound();
...
{
	if (sourceNumberOfNodes)
		THROW_AWAY(("source document has already been registered"));
...
{
	if (!sourceNumberOfNodes)
		THROW_AWAY(("source document must be registered before target document"));
...
int NodesManager::registerSubtree(DOMNode *node, bool isSource)
{
	...
	if (node == NULL)
		throw VersionManagerException("runtime-error", "NodesManager::registerSubtree", "node is NULL");
	...
	case DOMNode::ELEMENT_NODE:
	{
		...
		if (!myAtomicInfo.hasIdAttr)
		{
			...
			//SACHA
			XyUTF8Str _tagName(node->getNodeName());
			std::string tag = std::string(_tagName.localForm());
			myAtomicInfo.tag = tag;
		}
	...
	case DOMNode::TEXT_NODE:
		...
		myAtomicInfo.tag = "TEXT";
		if (XMLString::stringLen(node->getNodeValue()) > 0)
			myAtomicInfo.myWeight += (float)log((double)XMLString::stringLen(node->getNodeValue()));

		/* For text nodes containing only whitespaces and \13 chars, 
			 * set the flag 'isUnimportant'
			 */

		if (!XID_DOMDocument::isRealData(node))
			myAtomicInfo.isUnimportant = true;

		break;

	case DOMNode::CDATA_SECTION_NODE:
		myAtomicInfo.tag = "CDATA";
		break;
	...
	if (isSource)
		myAtomicInfo.myID = ++sourceNumberOfNodes;
	else
		myAtomicInfo.myID = ++resultNumberOfNodes;
	...
	for (unsigned int i = 0; i < nbChildren; i++)
	{
		...
		if (i < (nbChildren - 1))
			childAtomicInfo->nextSibling = childList[i + 1];
	}
	if (nbChildren)
		myAtomicInfo.firstChild = childList[0];

	// Store node in array

	if (isSource)
		v0node.push_back(myAtomicInfo);
	else
		v1node.push_back(myAtomicInfo);
...
int NodesManager::getBestCandidate(int v1nodeID, unsigned long selfkey)
{
	...
	if (fullListPointer == listOfCandidatesByParentLevelByHash[0].end())
		return 0;
	if (fullListPointer->second.v0node.size() == 0)
		return 0;
	...
	while (candidateRelativeLevel <= maxLevelPath)
	{
		...
		else
		{
			...
			else
			{
				...
				if (theList == listOfCandidatesByParentLevelByHash[0].end())
					return 0;
...
int NodesManager::FullBottomUp(int v1nodeID)
{
	...
	while (childID)
	{
		...
		if (childMatch)
		{
			...
			if (v0childParent)
			{
				if (weightByCandidate.find(v0childParent) == weightByCandidate.end())
					weightByCandidate[v0childParent] = childInfo.myWeight;
				else
					weightByCandidate[v0childParent] += childInfo.myWeight;
			}
		}
...
void NodesManager::ComputeWeakMove(int v0nodeID)
{
	...
	while (v0childID)
	{
		...
		if (childInfo.myEvent == AtomicInfo::NOP)
			oldChildValue.push_back(wSequence(index++, childInfo.myWeight));
		else
			oldChildValue.push_back(wSequence(0, 998000.0));
	...
	unsigned int i;
	for (i = 1; i < oldChildValue.size(); i++)
		vddprintf(("(%d, %.2f)  ", oldChildValue[i].data, oldChildValue[i].weight));
	vddprintf(("\n"));

	std::vector<wSequence> originalSequence;
	originalSequence.push_back(wSequence(-1, 997000.0));
	for (i = 1; i < oldChildValue.size(); i++)
		if (oldChildValue[i].data)
			originalSequence.push_back(oldChildValue[i]);
	...
	for (i = 1; i < finalSequence.size(); i++)
		vddprintf(("%d ", finalSequence[i].data));
	vddprintf(("\n"));
			...
			if ((child1) && (!v1Assigned(child1)) && (myChild1Info.nextSibling == 0))
			{
				...
				if ((domChild0->getNodeType() == DOMNode::TEXT_NODE) && (domChild1->getNodeType() == DOMNode::TEXT_NODE))
				{
					if ((myChild0Info.myEvent == AtomicInfo::DELETED) && (myChild1Info.myEvent == AtomicInfo::INSERTED))
					{
						...
						throw VersionManagerException("Internal Error", "Optimize()", "Update handler state is	inconsistant");
				}
			}
		}
	}
